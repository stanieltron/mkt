...
const rng = createRng(`quad-trades-${seed}`);
const path = buildRegimePath({ regime: "neutral", startPrice: 3000, steps: 3000, seed: `quad-price-${seed}` });
const openSchedule = buildOpenSchedule(path.length, 10000, `quad-trades-${seed}-opens`);
...
const holdOnlyEnd = startUSDC + ((startWETH * endPrice) / E18) / E12;

const resultPath = writeScenarioResult({
  scenarioName: "model-v1-neutrality-favorable-seed-978",
  payload: {
    model: "v1",
    scenarioType: "neutrality_favorable_single_seed",
    regime: "neutral",
    seed,
    leverage: 100,
    feeBps: 0,
