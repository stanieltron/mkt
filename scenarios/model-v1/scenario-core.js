...
const { LongOnlyKnockoutDexModelV1, STATUS, toE18, fmtUSDC6, fmtWETH18, fmtPriceE18, USDC_SCALE, E18 } = require("./dex-model");
...
function settleRemainingTrades(dex, liveTrades, counters) {
  ...
  while (liveTrades.size > 0) {
    if (liveTrades.size < snapThreshold) {
...
function runVerbose10TradesModelV1() {
  ...
  console.log("==================================================");
  console.log("Scenario: Verbose 10 Trades (Model V1)");
  ...
  const bottomLine = summarizeBottomLine(summary);
  printFinalSummary("Model V1", summary);
  const resultPath = writeScenarioResult({
    scenarioName: "model-v1-verbose-10-trades",
    payload: {
      model: "v1",
...
function simulateRandom10000TrendModelV1({
  regime,
  priceSeed,
  tradeSeed,
  targetTrades = 10_000,
...
function runRandom10000TrendModelV1({ regime, title, priceSeed, tradeSeed }) {
  const { summary, pathLength, targetTrades } = simulateRandom10000TrendModelV1({
    regime,
    priceSeed,
    tradeSeed,
  });
  ...
  console.log(`Model:    V1`);
  console.log(`PathPts:  ${pathLength}`);
  console.log(`Trades:   ${targetTrades}`);
  printFinalSummary("Model V1", summary);
  const resultPath = writeScenarioResult({
    scenarioName: `model-v1-random-10000-${regime}`,
    payload: {
      model: "v1",
...
module.exports = {
  runVerbose10TradesModelV1,
  runRandom10000TrendModelV1,
  simulateRandom10000TrendModelV1,
};
