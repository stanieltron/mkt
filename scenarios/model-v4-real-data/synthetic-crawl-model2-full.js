...
const END_PRICE = 19500;
const TARGET_TRADES = 78000;
...
async function runSyntheticCrawlModel2Full() {
  ...
  dex.mintUSDC(ADDR.trader, 100_000_000n * USDC_SCALE);
  ...
  const payload = {
    model: "v4",
    scenarioType: "synthetic-crawl-model2-full",
    scenarioLabel: "Synthetic crawl bridge: 78,000 sequential 100x long trades across a 6.5x baseline rise",
    config: {
  ...
  const filePath = writeScenarioResult({
    scenarioName: "model-v4-synthetic-crawl-model2-full",
    payload,
  });

  console.log("==================================================");
  console.log("Synthetic Crawl Model2 Full");
...
runSyntheticCrawlModel2Full().catch((err) => {
  console.error(err);
  process.exitCode = 1;
});
