...
async function runHistoricalReplayModel2() {
  ...
  const payload = {
    model: "v4",
    scenarioType: "historical-replay-model2-one-slice",
    scenarioLabel: "Historical ETH 1m replay, one sequential 100x long slice, protocol funded with 12 WETH",
    dataset: {
  ...
  const filePath = writeScenarioResult({
    scenarioName: "model-v4-historical-model2-one-slice-100x-12weth",
    payload,
  });

  console.log("==================================================");
  console.log("Historical Replay Model2: one sequential 100x long slice");
...
runHistoricalReplayModel2().catch((err) => {
  console.error(err);
  process.exitCode = 1;
});
