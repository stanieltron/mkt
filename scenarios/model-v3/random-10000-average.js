...
const { simulateRandom10000TrendModelV3 } = require("./scenario-core");
...
async function runMain() {
  ...
  for (const scenario of selectedScenarios) {
    ...
    const resultPath = writeScenarioResult({
      scenarioName: `model-v3-average-100-${scenario.key}`,
      payload: {
        model: "v3",
...
function runWorker() {
  ...
  const { summary } = simulateRandom10000TrendModelV3({
