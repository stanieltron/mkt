...
const { LongOnlyKnockoutDexModelV3, STATUS, toE18, fmtUSDC6, fmtWETH18, fmtPriceE18, USDC_SCALE, E18 } = require("./dex-model");
...
function setupDexSimple() {
  const dex = new LongOnlyKnockoutDexModelV3({ owner: ADDR.owner });
...
function setupDexWithTraders(traders, { leverage } = {}) {
  const dex = new LongOnlyKnockoutDexModelV3({ owner: ADDR.owner });
  if (leverage !== undefined) dex.leverage = BigInt(leverage);
...
function runVerbose10TradesModelV3() {
  ...
  console.log("==================================================");
  console.log("Scenario: Verbose 10 Trades (Model V3)");
  ...
  const bottomLine = summarizeBottomLine(summary);
  printFinalSummary("Model V3", summary);
  const resultPath = writeScenarioResult({
    scenarioName: "model-v3-verbose-10-trades",
    payload: {
      model: "v3",
...
function simulateRandom10000TrendModelV3({
  regime,
  priceSeed,
  tradeSeed,
  leverage = 300,
...
function runRandom10000TrendModelV3({ regime, title, priceSeed, tradeSeed }) {
  const { summary, pathLength, targetTrades } = simulateRandom10000TrendModelV3({
    regime,
    priceSeed,
    tradeSeed,
  });
  ...
  console.log(`Model:    V3`);
  console.log(`PathPts:  ${pathLength}`);
  console.log(`Trades:   ${targetTrades}`);
  printFinalSummary("Model V3", summary);
  const resultPath = writeScenarioResult({
    scenarioName: `model-v3-random-10000-${regime}`,
    payload: {
      model: "v3",
...
module.exports = {
  runVerbose10TradesModelV3,
  runRandom10000TrendModelV3,
  simulateRandom10000TrendModelV3,
};
