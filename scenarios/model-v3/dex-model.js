...
class LongOnlyKnockoutDexModelV3 {
  ...
  _wethFromUsdcCeil(usdcAmount6, priceE18) {
    ...
    const p = toBigInt(priceE18);
    const num = usdc6 * E12 * E18;
    return (num + p - 1n) / p;
  }
  ...
  _wethFromUsdcFloor(usdcAmount6, priceE18) {
    const usdc6 = toBigInt(usdcAmount6);
    const p = toBigInt(priceE18);
    return (usdc6 * E12 * E18) / p;
  }

  _usdcFromWeth(wethAmount18, priceE18) {
    const weth18 = toBigInt(wethAmount18);
    const p = toBigInt(priceE18);
    return ((weth18 * p) / E18) / E12;
  }

  _sellEthForUsdc(neededUsdc6, priceE18) {
    const need = toBigInt(neededUsdc6);
    if (need <= 0n) return { soldWETH: 0n, raisedUSDC: 0n };
    ...
    const raisedUSDC = this._usdcFromWeth(soldWETH, priceE18);
    this._set(this.wethBalances, this.dexAddress, this.balanceWETH(this.dexAddress) - soldWETH);
    this._set(this.usdcBalances, this.dexAddress, this.balanceUSDC(this.dexAddress) + raisedUSDC);
    return { soldWETH, raisedUSDC };
  }

  _buyEthWithUsdc(usdcAmount6, priceE18) {
    const spend = toBigInt(usdcAmount6);
    if (spend <= 0n) return { spentUSDC: 0n, boughtWETH: 0n };
    if (this.balanceUSDC(this.dexAddress) < spend) throw new Error("insufficient USDC");

    const boughtWETH = this._wethFromUsdcFloor(spend, priceE18);
    this._set(this.usdcBalances, this.dexAddress, this.balanceUSDC(this.dexAddress) - spend);
    this._set(this.wethBalances, this.dexAddress, this.balanceWETH(this.dexAddress) + boughtWETH);
    return { spentUSDC: spend, boughtWETH };
  }
  ...
  _freeUsdcBalance() {
    ...
    return poolUSDC - this.reservedMarginUSDC;
  }
  ...
  openTrade(sender, expectedPriceE18, toleranceBps, profitTargetBps) {
    ...
    this.reservedMarginUSDC += this.marginUSDC;
    return tradeId;
  }
  ...
  close(_caller, tradeId) {
    ...
    if (hitSL) {
      ...
      const availableUsdc = freeUsdc + t.marginUSDC;
      if (availableUsdc < t.marginUSDC) throw new Error("insufficient USDC");

      this.reservedMarginUSDC -= t.marginUSDC;
      const bought = this._buyEthWithUsdc(t.marginUSDC, p);
      ...
      return {
        ...
        payoutProfitWETH: 0n,
        payoutProfitUSDC: 0n,
        soldWETHForProfit: 0n,
        boughtWETHOnSL: bought.boughtWETH,
      };
    }

    t.status = STATUS.CLOSED_TP;

    const payoutUSDC = t.marginUSDC + t.targetProfitUSDC;
    let soldWETHForProfit = 0n;
    const freeUsdc = this._freeUsdcBalance();
    const availableUsdc = freeUsdc + t.marginUSDC;
    if (availableUsdc < payoutUSDC) {
      const need = payoutUSDC - availableUsdc;
      const sold = this._sellEthForUsdc(need, p);
      soldWETHForProfit = sold.soldWETH;
    }

    this.reservedMarginUSDC -= t.marginUSDC;
    if (this.balanceUSDC(this.dexAddress) < this.reservedMarginUSDC + payoutUSDC) throw new Error("insufficient USDC");
    this._transfer(this.usdcBalances, this.dexAddress, t.trader, payoutUSDC, "USDC");
    this.openNotionalUSDC -= t.notionalUSDC;
    this.openMarginUSDC -= t.marginUSDC;
    t.status = STATUS.CLOSED_TP;
