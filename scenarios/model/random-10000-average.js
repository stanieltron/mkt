...
const { simulateRandom10000TrendModelV4 } = require("./scenario-core");
...
function addSummary(acc, summary) {
  ...
  acc.openedShorts += summary.openedShorts;
  acc.openFailInsufficientLiquidity += summary.openFailInsufficientLiquidity;
  acc.openFailNoOffset += summary.openFailNoOffset;
  ...
  acc.shortTradePnl += summary.shortTradePnl;
...
function printAverageReport(label, acc) {
  ...
  const avgOpenedShorts = acc.openedShorts / runs;
  const avgOpenFailInsufficientLiquidity = acc.openFailInsufficientLiquidity / runs;
  const avgOpenFailNoOffset = acc.openFailNoOffset / runs;
  ...
  const avgShortTradePnl = avgBigInt(acc.shortTradePnl, runs);
  ...
  console.log(`avgShortsOpened:                   ${fmtNum(avgOpenedShorts, 2)}`);
  console.log(`avgOpenFailInsufficientLiquidity:  ${fmtNum(avgOpenFailInsufficientLiquidity, 2)}`);
  console.log(`avgOpenFailNoShortOffset:          ${fmtNum(avgOpenFailNoOffset, 2)}`);
  ...
  console.log(`avgShortsRealizedPnL:              ${fmtUSDC6(avgShortTradePnl)} USDC`);
...
async function runMain() {
  ...
  for (const scenario of selectedScenarios) {
    ...
    const resultPath = writeScenarioResult({
      scenarioName: `model-v4-average-100-${scenario.key}`,
      payload: {
        model: "v4",
...
function runWorker() {
  ...
  const { summary } = simulateRandom10000TrendModelV4({
