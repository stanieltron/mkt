...
class LongOnlyKnockoutDexModelV2 {
  ...
  close(_caller, tradeId) {
    ...
    const usdcBal = this.balanceUSDC(this.dexAddress);
    if (usdcBal < payoutUSDC) {
      const need = payoutUSDC - usdcBal;
      const sold = this._sellEthForUsdc(need, p);
      soldWETHForProfit = sold.soldWETH;
    }

    if (this.balanceUSDC(this.dexAddress) < payoutUSDC) throw new Error("insufficient USDC");
